...
class DownloadService {
  ...
  private async checkNetworkConnectivity(): Promise<boolean> {
    // Always return true and let the actual search handle connectivity issues
    // This prevents false negatives from pre-flight checks
    return true;
  }
  ...
  async searchInternetArchive(movieTitle: string): Promise<SearchResult> {
    ...
    // Skip pre-flight connectivity check - let actual search handle it
    ...
    try {
      ...
      for (const searchQuery of searchQueries) {
        ...
        try {
          ...
          // If it's a network error on the first query, show user-friendly message
          if (queryError.name === 'AbortError' || 
              queryError.message?.includes('Network request failed') || 
              queryError.message?.includes('Failed to fetch')) {
            console.log('[DownloadService] Network connectivity issue detected');
          }
      ...
      // No additional fallback needed - already covered in main search queries

      if (bestResults.length === 0) {
        this.showToast('Movie Not Found', `No movie files found for "${cleanTitle}". Try different spelling or check if the movie is available on Internet Archive.`);
      ...
      this.showToast('Search Error', errorMessage);

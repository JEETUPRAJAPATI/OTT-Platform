...
class DownloadService {
  ...
  async searchInternetArchive(movieTitle: string): Promise<SearchResult> {
    ...
    try {
      ...
      // Improved search strategies based on working examples
      const searchQueries = [
        // Best working pattern - search in title OR description, exclude songs/trailers
        `(title:("${cleanTitle}")+OR+description:("${cleanTitle}"))+AND+mediatype:(movies)+NOT+title:(song+OR+full-song+OR+trailer+OR+video+OR+clip)`,
        
        // Add "movies" keyword to help find movies - this works well
        `${encodedTitle}+movies+AND+mediatype:(movies)`,
        `${encodedTitle}+movies+hindi+AND+mediatype:(movies)`, // For Indian movies
        
        // Original working patterns
        `title:(${encodedTitle})+AND+mediatype:(movies)`, // Direct title search
        `${encodedTitle}+movie+AND+mediatype:(movies)`, // Generic movie search
        
        // Broader searches as fallback
        `${encodedTitle}+AND+mediatype:(movies)+AND+format:(MPEG4+OR+Matroska+OR+Ogg Video)`,
      ...
      // If no results found, try one more fallback with "movies" keyword
      if (bestResults.length === 0) {
        this.logNetwork('No results found, trying fallback with "movies" keyword');
        
        try {
          const fallbackQuery = `${encodedTitle}+movies+AND+mediatype:(movies)`;
          const fallbackUrl = `https://archive.org/advancedsearch.php?q=${fallbackQuery}&fl=identifier,title&rows=10&page=1&output=json&sort[]=downloads+desc`;
          ...
          if (response.ok) {
            ...
            if (responseText && responseText.trim().length > 0) {
              const data = JSON.parse(responseText);
              if (data.response && Array.isArray(data.response.docs) && data.response.docs.length > 0) {
                bestResults = data.response.docs.filter((doc: any) => {
                  const title = doc.title?.toLowerCase() || '';
                  const identifier = doc.identifier?.toLowerCase() || '';
                  ...
                  if (title.includes('twitter') || title.includes('@') ||
                      identifier.includes('twitter') || identifier.includes('odysee') ||
                      title.includes('podcast') || title.includes('radio')) {
                    return false;
                  }
                ...
                this.logNetwork('Fallback search found results', fallbackUrl, { count: bestResults.length });
              }
            }
          }
        } catch (fallbackError) {
          this.logNetwork('Fallback search failed', 'fallback', fallbackError);
        }
      }

      if (bestResults.length === 0) {
        this.showToast('Movie Not Found', `No movie files found for "${cleanTitle}". Try adding "movies" to your search (e.g., "${cleanTitle} movies") or check the spelling.`);
